...
(() => {
  ...
  var import_string_similarity_js = __toESM2(require_string_similarity());
  ...
  async function bfsIdForPostalCodeAndCity(postalCode, city, year) {
    ...
    for (const location2 of locations) {
      const matchingZip = location2.ZipCodes?.find((z) => z.postalCode === postalCode);
      if (matchingZip) {
  ...
  var siteConfigs = {
    "www.newhome.ch": {
      ...
      resultElementId: "swiss-tax-calculator-result",
      // ID for the injected element
      resultContainerClass: "formatted-value__value",
      // Class to mimic existing styles
      insertResult: true
      // Flag to indicate insertion point relative to price element
    },
    "www.immoscout24.ch": {
    ...
    default: {
      ...
      logUnsupported: true
      // Example flag to log if a site isn't explicitly configured
    }
  };
  var sentUrls = /* @__PURE__ */ new Set();
  ...
  async function calculateTaxesAsync(postalCode, city) {
    ...
    try {
              ...
              persons: [
                {
                  age: 30,
                  confession: "roman",
                  // Consider making confession configurable or using 'other'/'none'
                  income: 1e5,
                  incomeType: "gross",
                  pkDeduction: 5e3
                }
      ...
      const newTaxInput = { ...taxInput, locationId: bfsId, cantonId };
      ...
      return {
        ...
        old: oldTaxesResult?.taxesTotal ?? 0
        // Default old taxes to 0 if calculation failed or wasn't possible
      };
    } catch (error) {
  ...
  function displayTaxResults(anchorElement, calculationResult, config) {
    ...
    const translations = {
      ...
      changePreferences: {
        ...
        de: "Steuereinstellungen \xE4ndern",
        fr: "Modifier les pr\xE9f\xE9rences fiscales",
      ...
      setLocationMessage: {
        ...
        fr: "(D\xE9finissez votre emplacement actuel dans les options de l'extension pour voir la diff\xE9rence)",
